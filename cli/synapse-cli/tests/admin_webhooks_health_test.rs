...
/// Matches `mock-server.rs`'s `DEFAULT_ADMIN_TOKEN` — `admin webhooks
/// health*` is behind `admin_auth` on the real server, so the mock now
/// rejects requests that don't send `Authorization: Bearer <this>`.
