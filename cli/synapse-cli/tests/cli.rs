...
/// Matches `mock-server.rs`'s `DEFAULT_ADMIN_TOKEN`. Every `admin ...`
/// subcommand hits a route behind `admin_auth` on the real server, so the
/// mock (spawned via `MockServer::spawn` below) now rejects requests that
/// don't send `Authorization: Bearer <this>`. Tests that stub responses
/// with `mockito` instead don't need this — mockito doesn't validate
/// headers unless a test explicitly asks it to.
...
#[test]
fn reconciliation_run_table_mode_happy_path() {
    let server = MockServer::spawn("happy");
    let base_url = server.base_url();
