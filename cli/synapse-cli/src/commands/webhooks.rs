        ...
        WebhooksCommand::HealthGet { id, json } => {
            ...
            match client.get::<EndpointHealth>(&path).await {
                Err(synapse_sdk::SynapseError::NotFound(_)) => {
                    // Built via `CliError::NotFound` rather than a plain
                    // `bail!` so the typed error survives to `main.rs` —
                    // `bail!` here would erase it into an untyped
                    // `anyhow::Error`, which `handle_anyhow_error` can no
                    // longer downcast to map onto `EXIT_NOT_FOUND`.
...
mod tests {
    ...
    async fn health_list_happy_path_deserialises() {
        ...
        let client = AdminClient::new(&server.url(), "test-key");
