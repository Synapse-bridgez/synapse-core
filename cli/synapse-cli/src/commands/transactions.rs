...
pub enum TransactionsSubcommand {
    ...
    ///
    /// Calls GET /export (an admin-only route — requires the same credential
    /// as `synapse admin ...` commands). Downloads a CSV or JSON export.
    ///
    /// Examples:
...
pub async fn run(cmd: TransactionsSubcommand, base_url: &str, api_key: &str) -> Result<()> {
    ...
    match cmd {
        ...
        TransactionsSubcommand::Export {
            ...
            let bytes = admin_client.get_bytes("/export", &query).await?;
