...
async fn test_api_versioning_headers() {
    ...
    app_state.pool_manager = synapse_core::db::pool_manager::PoolManager::new(&database_url, None)
        .await
        .unwrap();
    app_state.horizon_client = synapse_core::stellar::HorizonClient::new(
        "https://horizon-testnet.stellar.org".to_string(),
    );
    app_state.feature_flags = synapse_core::services::feature_flags::FeatureFlagService::new(pool.clone());
