...
async fn test_set_local_prevents_connection_reuse_leak() {
    ...
    for (tid, name) in [(tenant_a, "TenantA"), (tenant_b, "TenantB")] {
        sqlx::query("INSERT INTO tenants (tenant_id, name, api_key_hash, webhook_secret, stellar_account, rate_limit_per_minute, is_active) VALUES ($1,$2,$3,pgp_sym_encrypt('', $4),'',60,true)")
            .bind(tid)
            .bind(name)
...
async fn test_no_context_fails_closed() {
    ...
    sqlx::query("INSERT INTO tenants (tenant_id, name, api_key_hash, webhook_secret, stellar_account, rate_limit_per_minute, is_active) VALUES ($1,'TenantA',$2,pgp_sym_encrypt('', $3),'',60,true)")
        .bind(tenant_a)
        .bind(synapse_core::db::queries::hash_api_key(&Uuid::new_v4().to_string()))
        .bind(synapse_core::db::queries::tenant_secret_key())
        .execute(&pool)
