...
impl<'a> AdminWebhookReplay<'a> {
    ...
    ///
    /// Calls `GET /admin/webhooks/failed` with the admin key (`Authorization: Bearer`).
    ...
    ///
    /// Calls `POST /admin/webhooks/replay/:id` with the admin key (`Authorization: Bearer`).
    ...
    ///
    /// Calls `POST /admin/webhooks/replay/batch` with the admin key (`Authorization: Bearer`).
    ///
    /// # Per-item results
...
mod tests {
    ...
    async fn list_failed_returns_webhooks_on_200() {
        ...
        Mock::given(method("GET"))
            ...
            .and(header("Authorization", "Bearer admin-test-key"))
    ...
    async fn list_failed_uses_admin_key_not_public_key() {
        ...
        // Mock only matches Authorization: Bearer; success proves the right header was sent.
        assert!(
            result.is_ok(),
    ...
    async fn replay_uses_admin_key_not_public_key() {
        ...
        // Mock requires Authorization: Bearer; success confirms correct header.
        assert!(
            result.is_ok(),
            "expected Ok with admin key, got: {:?}",
            result
