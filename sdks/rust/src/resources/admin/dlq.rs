...
impl<'a> AdminDlq<'a> {
    ...
    ///
    /// Calls `GET /dlq` with the admin key (`Authorization: Bearer` header).
    ...
    ///
    /// Calls `POST /dlq/:id/requeue` with the admin key (`Authorization: Bearer` header).
...
mod tests {
    ...
    async fn list_uses_admin_key_not_public_key() {
        ...
        // If the test passes the mock matched, meaning Authorization: Bearer was sent, not X-API-Key.
        assert!(
            result.is_ok(),
    ...
    async fn requeue_uses_admin_key_not_public_key() {
        ...
        // Mock only matches Authorization: Bearer; success proves the right key was sent.
        assert!(
            result.is_ok(),
            "expected Ok with admin key, got: {:?}",
            result
